    ...
    <div class="container" align="center">
        ...
        <table border='1' cellpadding='0' cellspacing='0' class="prebook_result" style="font-size: 11pt;">
            ...
            <tr style="font-weight: bold;">
                <td>序号</td>
                <td width="125">业务员</td>
                <td width="325">所属网点</td>
                <td>操作</td>
            </tr>
            <tbody id="clerkListTbody"></tbody>
            <c:forEach items="${clerks}" var="clerk" varStatus="i">
                <tr id="clerk_${clerk.clerk_id}">
                    ...
                    <td class="edit_td"><a
                            href="${pageContext.request.contextPath}/admin/toModifyClerk.form?clerkId=${clerk.clerk_id}">编辑</a>&nbsp;
                        <a href="${pageContext.request.contextPath}/admin/deleteClerk.form?clerkId=${clerk.clerk_id}"
                           onclick="javascript: return confirm('你确定的要删除   ${clerk.clerk_username} 业务员记录？')">删除</a>
                    </td>
                </tr>
            </c:forEach>
        </table>
...
<script type="text/javascript">
    function deleteClerk(clerkId, clerk_username) {
        if (!confirm('确定要删除['+clerk_username+']业务员吗？')){
            return false;
        }

        var url = contextPath + '/admin/deleteClerk.form';
    ...
    $(document).ready(function() {
        ...
        var url = contextPath + '/admin/listClerk.form';
        $.post(url, {}, function (json) {
            if(json.code == '0000'){
                var clerkList = json.data.clerkList;
                var html;
                for(var i=0; i<clerkList.length; i++) {
                    var clerk = clerkList[i];
                    html += '<tr>';
                    html += '<td>'+(i+1)+'</td>';
                    html += '<td>'+clerk.clerk_username+'</td>';
                    html += '<td>'+clerk.department.department_name+'</td>';
                    html += '<td>';
                    html += '<a href="'+contextPath+'/admin/toModifyClerk.form?clerkId='+clerk.clerk_id+'">编辑</a>';
                    html += '&nbsp;&nbsp;';
                    html += '<a href="javascript:;" onclick="deleteClerk('+clerk.clerk_id+',\''+clerk.clerk_username+'\')">删除</a>';
                    html += '</td>';
                    html += '</tr>';
                }
                $('#clerkListTbody').html(html);
            }else {
                alert(json.msg);
            }
        });
