...
public class SalesmanController {
    ...
    @ResponseBody
    @RequestMapping("/listPrebook.form")
    @Transactional(readOnly = true)
    public JsonResult listPrebook(Integer clerkId) {
        JsonResult jsonResult = JsonResult.getInstance();
        try {
            List<Prebook> prebookList = salesmanService.findPrebookByClerkId(clerkId);
            jsonResult.getData().put("prebookList", prebookList);
        } catch (Exception e) {
            e.printStackTrace();
            jsonResult = JsonResult.getFailResult(BaseErrorMsg.ERROR_API_FAIL);
        }
        System.out.println("listPrebook reponse=" + JSONObject.toJSONString(jsonResult));
        return jsonResult;
    }
    ...
    public JsonResult modifyPrebook(String status, Integer prebookId) {
        JsonResult jsonResult = JsonResult.getInstance();
        try {
